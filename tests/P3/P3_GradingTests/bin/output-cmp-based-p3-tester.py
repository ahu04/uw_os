...
def run_command(run, timeout_length, verbosity):
    if 'P3_ALTER_PATH' in os.environ:
        path_key = os.environ['P3_ALTER_PATH']
        path = f'PATH={path_key}'
    else:
        path = ''
    if args.non_sess_leader:
        cmd = f'script -E never -qc \'tty; sleep 0.1; {path} {non_sess_leader_path} ./{binary_file}\' /dev/null'
    else:
        cmd = f'script -E never -qc \'tty; sleep 0.1; {path} ./{binary_file}\' /dev/null'
...
def handle_outcomes(stdout_expected, stdout_actual, stderr_expected, stderr_actual, rc_expected, rc_actual,
    ...
    if len(extra_err_msg) != 0:
        print('RESULT failed\nSome error happened during testing: ')
...
parser.add_argument("--non_sess_leader", action="store_true", help="By default, wsh is launched as a session leader. If the flag is set, wsh is launched as a non session leader. ")
...
non_sess_leader_path = '/home/yurun/P3/GRADING/p3-grader/bin/launcher'
